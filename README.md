...
| `seedkernel-wasm/bundle` | `verifyBundle` — the one call that unpacks and checks an offered bundle (`peekMeta`); `hybridAuthorKeysFromSeed`/`hybridAuthorId` derive this browser's own author id from its key seed; `genesisHash` for the caller id an app key hashes to. Authoring is offline (`scripts/build-app-bundle.mjs`, `authorBundle`), the same API seedstore's build uses. `scripts/smoke.mjs` also reaches the module's lower-level primitives, to forge and tamper with bundles the verifier must reject — a test path, not the shell's |
