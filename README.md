...
routing, signed bundles, the channel handshake — are documented in seedkernel:
...
| `scripts/smoke.mjs` | Headless regression test: boots two shells over the transport bundle's channel seam, round-trips a message through a real chat-app-v1.wasm, and round-trips an offer through the offers app. Run it after a seedkernel update. |
...
`vendor/host` and the shell fails in ways that look like seedkernel bugs. If a rebuild
