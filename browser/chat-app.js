...
/** Everything a chat app reaches (`manifest.guest.requires`, §12.2, §12.10): the
 *  network, and nothing else — no host service at all.
 *
 *  Naming `_net` is not an indulgence but the whole of how an app sends. The host has
 *  no send: the socket driver holds descriptors and the transport is a guest that serves
 *  the local service name `_net`, so a message reaches a peer by an app CALLING that id.
 *  A chat app that could not name `_net` could receive chat and never send it, and the
 *  shell would need a second app of its own to do the sending — the same authority, one
 *  indirection further from the thing that uses it. Holding no host service is the other
 *  half: an operator is asked who may *be* the network (`link`), sign as the node
 *  (`node`) or keep a store (`fs`), and a chat app names none of them.
 *
 ...
 *  accepts — see `peekMeta`. The bundle's own modules stay ungated like `crypto`
 *  (seedkernel §12.1) and appear in no signed list at all. */
 ...
 *
 *  The reach check is not decoration, and it is an equality rather than a subset test for
 *  that reason. An Offer arrives from a peer and is installed on one click of a row showing
 *  a name, a version and an author; `guest.requires` is the only place the bundle's reach
 *  is written down, and nothing else on this path reads it. A subset test would pass an app
 *  that calls a co-resident guest this shell never heard of, or one whose guest holds
 *  `node`, `fs` or `link`, and the consent prompt would look exactly the same. */
