// Minimal WebSocket broadcast hub — the signaling rendezvous for the kernel's
// RtcNetwork (seedkernel `host/net-rtc.ts`, §12.7). It lives here rather than in
// the kernel repo because it is a *deployment* concern, not runtime surface: the
// kernel ships no server and its own tests signal in-process. Nothing about it is
// chat-specific, though — seed store points at this file too, as can anything
// else standing up a WebRTC mesh.
//
